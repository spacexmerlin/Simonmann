...
// Über-mich-Bio als Swipe-Slider: ziehen oder Punkte anklicken, um zwischen
// den 4 Absätzen zu wechseln.
function initBioSlider() {
  const slider = document.getElementById("bio-slider");
  if (!slider) return;
  const track = slider.querySelector(".bio-slider-track");
  const slides = Array.from(slider.querySelectorAll(".bio-slide"));
  const dots = Array.from(slider.querySelectorAll(".bio-dot"));
  ...
  function render(withOffsetPx) {
    const offset = withOffsetPx || 0;
  ...
  function endDrag() {
    ...
    const threshold = 60;
    if (dragOffset < -threshold && index < slides.length - 1) index += 1;
    else if (dragOffset > threshold && index > 0) index -= 1;
    dragOffset = 0;
    render();
  }
